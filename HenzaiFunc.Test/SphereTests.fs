...
let unitSphereBoundingTest () =
    ...
    Assert.Equal(Vector3(-1.0f,-1.0f,-1.0f), aabb.PMin)
    Assert.Equal(Vector3(1.0f,1.0f,1.0f), aabb.PMax)

[<Fact>]
let sphereBoundingTest () =
    let unitSphere = Sphere(Vector3(5.0f,5.0f,-5.0f),1.0f)
    let aabb = unitSphere.AsBoundable.GetBounds
    Assert.Equal(Vector3(4.0f,4.0f,-6.0f), aabb.PMin)
    Assert.Equal(Vector3(6.0f,6.0f,-4.0f), aabb.PMax)
